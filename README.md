...
- **Real Factorio icons inline.** Tags inside labels (`[item=iron-plate]`,
  ...
  their original positions, hot-linked from a community icon repo via
  jsDelivr (see [Icons](#icons)).
- **Components panel for any blueprint.** Selecting a single blueprint shows
  ...
  legendary indicator in the corner), sorted by count desc. Click a tile to
  light up every matching entry in the JSON pane and scroll to the first
  occurrence; click the same tile again to cycle to the next match. Esc
...
- **100% client-side.** No server, no backend, no analytics. The entire app
  ...
  [pako](https://github.com/nodeca/pako) for zlib. Works offline once the
  page and the icon CDN are cached.
...
- `src/icons-manifest.js` — auto-generated by
  `scripts/build-icons-manifest.mjs`; maps `category_name` keys to jsDelivr
  URLs.
...
- `scripts/build-icons-manifest.mjs` — fetches and transforms the upstream
  icons manifest. Run with `npm run icons:update`.
